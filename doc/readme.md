...
* 2x EC11 旋转编码器及配套旋钮帽  
  > **⚠️ 注意：** 本项目 SOUND VOLTEX 旋钮部分使用的是**旋转编码器（Rotary Encoder）**，输出数字脉冲信号。请勿将其与输出模拟信号的传统电位器（Potentiometer）混淆。
  
  在编码器的选型上，存在两种主要的装配方案。综合游玩手感与装配成本，**本项目强烈推荐采用方案 A（第三方编码器去段落改造）**。
  
  **方案 A：第三方 EC11 编码器 + 物理改造（作者推荐）**  
  由于 ALPS 进口编码器成本高昂，使用常规的第三方/国产 EC11 编码器是极佳的替代方案。
  ...
  **旋钮帽选购提示：** EC11 编码器的金属转轴通常分为**半圆轴（D型轴）**与**梅花轴（齿形轴）**两种规格。购买配套的金属旋钮帽时，请务必确认旋钮帽内部的孔型与您采购的编码器轴体形状完全一致，否则将无法正常安装。
* 1x ALPS B10K 75mm 滑动电位器以及1个滑动电位器推子帽  
与旋转编码器不同，在选购滑动电位器时请务必购买 ALPS 原装进口版本的型号，否则游玩体验会受到较大负面影响。
